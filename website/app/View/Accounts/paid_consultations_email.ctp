...
<section class=" my_private_messages-page paid_consultations_email-page  page ">
   ...
   // $this->Session->setFlash('Thanks for your payment.');
    ...
    <article class="marge">
	<h1 class="">  <?= __('Consultations payantes par Email') ?></h1>
	
	<?php 
	$userRole = $this->Session->read('Auth.User.role'); 
	
	//echo"<br>userRole=".$this->Session->read('Auth.User.role');
	
	if($userRole=="agent")
	    echo __("Pour les consultations payantes par Email, vous avez choisi un délai de réponse de 6h, 12h, 24h ou 48h via la page “Mon profil”. Une fois ce délai dépassé, votre client sera automatiquement remboursé et vous ne pourrez plus prendre connaissance de cette consultation pour y répondre, ce dernier sera malgré tout libre de relancer une nouvelle consultation par Email. Plus le délai de réponse est court, plus vous favorisez l'engagement client, sa confiance et la possibilité qu'il vous consulte à nouveau. Si vous ne répondez pas, votre image sera entachée.");
	else
	   echo __("Vos consultations payantes par Email sont répertoriées sur la page ci-dessous, vous trouverez un classement par LiviMaster, contenant l'ensemble de vos demandes ou messages auprès de chacun d'entre eux ainsi que les réponses de ces derniers. Si un LiviMaster ne vous a pas répondu dans le délai annoncé par ses soins sur sa page, vous êtes automatiquement re-crédités et libre à vous de relancer une nouvelle consultation.");
