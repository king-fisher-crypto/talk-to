            ...
            <?php else: ?>
                <table class="table table-striped table-hover table-bordered">
                    ...
                    <tr>
                        ...
                        <th><?php echo __('HiPay Transaction'); ?></th>
                    ...
                    <?php 
                        ...
                        <tr>
                            ...
                            <td><?php
                                ...
                                echo $row['hipaylogs']['date_add'].'<br/>';

                                if (strtolower($row['hipaylogs']['status']) == 'ok')
                                    echo '<span class="badge badge-success">'.$row['hipaylogs']['operation'].': '.$row['hipaylogs']['status'].'</span><br/>';
                                else
                                    echo '<span class="badge badge-error">'.$row['hipaylogs']['operation'].': '.$row['hipaylogs']['status'].'</span><br/>';

                                echo 'Hipay trans: '.$row['hipaylogs']['transaction'];
                            ...
                            <td><?php 
								...
								if($row['Order']['valid'] == 2){
									echo $this->Time->format(Tools::dateUser($this->Session->read('Config.timezone_user'),$row['hipaylogs']['date_upd']),'%d %B %Y %H:%M');
								}else{
                            ...
                            /*
									...
									echo $this->Form->button('<i class="glyphicon glyphicon-eye icon_margin_right_5 "></i> '.__('Impayé'),
															array(
																...
																'href' => $this->Html->url(array('controller' => 'paymenthipay', 'action' => 'declarer_impaye'),true),
								...
								if($row['Order']['valid'] == 2)
									echo $this->Form->button('<i class="glyphicon glyphicon-eye icon_margin_right_5 "></i> '.__('Soldé'),
															array(
																'class' => 'btn btn-pink btn-pink-modified btn-small-modified mb0 payment_valid',
																'href' => $this->Html->url(array('controller' => 'paymenthipay', 'action' => 'declarer_valide'),true),
																'payment' => $row['Order']['id'],
																'label' => '',
																'type'  => 'button'
															)
								...
								if($row['Order']['valid'] != 3)
									echo $this->Form->button('<i class="glyphicon glyphicon-eye icon_margin_right_5 "></i> '.__('Rembourser'),
															array(
																...
																'href' => $this->Html->url(array('controller' => 'paymenthipay', 'action' => 'declarer_rembourse'),true),
																'payment' => $row['Order']['id'],
																'label' => '',
																'type'  => 'button'
															)
