                ...
                <?php foreach ($orders as $k => $row): ?>
                    <tr>
                        ...
                        <td>
                            <?php if ((int)$row['Order']['valid'] == 0): ?>
                                <div class="btn-group margin-left">
                                    ...
                                    <ul class="dropdown-menu">
                                        ...
                                        '<span class="icon-check"></span>'.__('Valider le coupon'),
                                        array('controller' => $this->request->controller, 'action' => 'confirm', 'admin' => true, 'id' => $row['Order']['id']),
                                        array('escape' => false),
                                        __('Voulez-vous vraiment valider le coupon "'.$row['Order']['reference'].'" ? (Le compte client sera crédité immédiatemment)')
                                    ); ?></li>
                                        ...
                                        <li><?php echo $this->Html->link(
                                                ...
                                                array('escape' => false),
                                                __('Voulez-vous vraiment SUPPRIMER le coupon "'.$row['Order']['reference'].'" ? (Action irréversible)')
                                            ); ?></li>
							...
							if (($row['Order']['total'] < 0 && !$row['Order']['product_id'] && $row['Order']['valid']) || !$row['Order']['valid']): ?>
                                <div class="btn-group margin-left">
                                    <a class="btn btn dropdown-toggle" data-toggle="dropdown" href="#"><?php echo __('Actions'); ?><span class="caret"></span></a>
                                    <ul class="dropdown-menu">

                                        <li><?php echo $this->Html->link(
                                        '<span class="icon-check"></span>'.__('Valider la carte cadeau'),
                                        array('controller' => $this->request->controller, 'action' => 'validgift', 'admin' => true, 'id' => $row['Order']['id']),
                                        array('escape' => false),
                                        __('Voulez-vous vraiment valider le coupon "'.$row['Order']['reference'].'" ? (Le compte client sera crédité immédiatemment)')
                                    ); ?></li>

                                        <li><?php echo $this->Html->link(
                                                '<span class="icon-remove"></span>'.__('Annuler le cadeau'),
                                                array('controller' => $this->request->controller, 'action' => 'removegift', 'admin' => true, 'id' => $row['Order']['id']),
                                                array('escape' => false),
                                                __('Voulez-vous vraiment SUPPRIMER le coupon "'.$row['Order']['reference'].'" ? (Action irréversible)')
                                            ); ?></li>
