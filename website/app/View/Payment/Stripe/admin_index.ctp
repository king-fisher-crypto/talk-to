            ...
            <?php else: ?>
                <table class="table table-striped table-hover table-bordered">
                    ...
                    <tr>
                        ...
                        <th><?php echo __('Stripe Transaction'); ?></th>
						<th><?php echo __('ID Panier'); ?></th>
                        <th><?php echo $this->Paginator->sort('Order.total', __('Date paiement')); ?></th>
						<th><?php echo $this->Paginator->sort('Order.total', __('Date modification')); ?></th>
                    ...
                    <?php 
                        ...
                        <tr>
                            ...
                            <td><?php
                                ...
                                echo 'Stripe id: <b>'.$row['stripelogs']['id'].'</b><br/>';

                               echo 'Stripe payment: <b>'.$row['stripelogs']['payment_method'].'</b>';


                                ?></td>
							<td><?php echo $row['Order']['cart_id']; ?></td>
							...
							<td><?php 
									...
									echo $this->Time->format(Tools::dateUser($this->Session->read('Config.timezone_user'),$row['stripelogs']['date_upd']),'%d %B %Y %H:%M');
							...
							<td><?php
									...
									echo $this->Form->button('<i class="glyphicon glyphicon-eye icon_margin_right_5 "></i> '.__('Impayé'),
															array(
																...
																'href' => $this->Html->url(array('controller' => 'paymentstripe', 'action' => 'declarer_impaye'),true),
																'payment' => $row['Order']['id'],
																'label' => '',
																'type'  => 'button'
															)
														);
								if($row['Order']['valid'] == 2){
									echo $this->Form->button('<i class="glyphicon glyphicon-eye icon_margin_right_5 "></i> '.__('Soldé'),
															array(
																'class' => 'btn btn-pink btn-pink-modified btn-small-modified mb0 payment_valid',
																'href' => $this->Html->url(array('controller' => 'paymentstripe', 'action' => 'declarer_valide'),true),
																'payment' => $row['Order']['id'],
																'label' => '',
																'type'  => 'button'
															)
									...
									echo $this->Form->button('<i class="glyphicon glyphicon-eye icon_margin_right_5 "></i> '.__('Declarer Incident'),
															array(
																...
																'href' => $this->Html->url(array('controller' => 'paymentstripe', 'action' => 'declarer_incident'),true),
																'payment' => $row['Order']['id'],
																'label' => '',
																'type'  => 'button'
															)
								...
								if($row['Order']['valid'] != 3)
									echo $this->Form->button('<i class="glyphicon glyphicon-eye icon_margin_right_5 "></i> '.__('Rembourser'),
															array(
																...
																'href' => $this->Html->url(array('controller' => 'paymentstripe', 'action' => 'declarer_rembourse'),true),
																'payment' => $row['Order']['id'],
																'label' => '',
																'type'  => 'button'
															)
