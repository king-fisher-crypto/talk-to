...
<section class="subscription-page a-historylostcall-page page jswidth">
    ...
    <article>
	<h1 class="">  <?= __('Mes Chats perdus') ?></h1>
		<?= __("Par mesure de sécurité, le mode Chat est automatiquement désactivé après 3 Chats perdus, il vous appartient ensuite de réactiver ce mode. Nous attirons votre attention sur la nécessité de répondre à vos Chats et à l'image que vous donnez en ne répondant pas, pouvant créer un désengagement de vos clients.") ?>	
    </article>
	...
	<?php else : ?>
    	<div  class="overflow jswidth">
			...
			<th class="desactivation"><div style="display:inline-block;text-align:left;"><?php echo __('Désactivation<br/>mode Chat'); ?></div></th> 
