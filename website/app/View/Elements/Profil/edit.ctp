...
//    if (strpos($field, "langs")>-1) continue;
	...
	case"address":
	    ...
	    break;
	case"email":
//	    $value = '<textarea name="data[Account][email]" placeholder="" rows="1" class="address" cols="40" maxlength="200" id="AccountEmail" oninput=\'this.style.height = "";this.style.height = this.scrollHeight + "px"\' style="word-break: break-all;">degrefinance.enternaiment@protonmail.com</textarea>';
	    
	   $value = $this->Form->input('email',
		    ['type' => 'textarea', 'label' => false, 'placeholder' => '', 'escape' => false,
		'class' => 'address ', 'rows' => '2', 'cols' => '40']);
	  break;
    ...
    <div class="field_bar <?= $field ?>">
        <div class="label"><span><?= $label ?></span><?php if(in_array($field,$fields_mandatory)){ ?><span class="asterisk">&#65121;</span>
	<?php } ?>
	</div>
...
<div class="obligatoires p24 t18 m14"><?=__('Les champs contenant une étoile sont obligatoires.')?></div>
 

<?php
echo $this->Form->end(array('label' => __('enregistrer'), 'class' => 'btn xlarge h85b modifier white  up_case',
