var nx_adminmenublocklink = {

    init : function(){
        $('.edit_block_link').unbind('click').click(function(){
            var postVars = {MenuBlockLang: {}};

            postVars.MenuBlockLang['menu_block_id'] = $('input[id=MenuBlockLangMenuBlockId]').val();

            var i = 0;
            $('.tab-pane.tab-block').each(function(){
                var lang = $(this).attr('lang');
                postVars.MenuBlockLang[i] = {};

                postVars.MenuBlockLang[i]['lang_id'] = $(this).find('input[id=MenuBlockLang'+lang+'LangId]').val();
                postVars.MenuBlockLang[i]['title'] = $(this).find('input[id=MenuBlockLang'+lang+'Title]').val();
                i++;
            });
            nxAdminMain.ajaxRequest($(this).attr('href'), postVars, function(json){
                if(json.return){
                    ...
                    nx_adminmenublocklink.updateSelectLink(json.update);
        ...
        $('#select_block_link').unbind('change').change(function(){
            ...
            if($(this).val() !== ''){
                ...
                nx_adminmenublocklink.updateLinkAction($(this).attr('update'), $(this).val());
                //On va chercher les datas pour le bloc sélectionner
                nx_adminmenublocklink.getDataLink($(this).attr('update'), $(this).val());
            }
        });
    ...
    updateSelectLink: function(url){
        nxAdminMain.ajaxRequest(url, {mode: 'menu_block_link'}, function(json){
            if(json.return){
...
$(document).ready(function(){ nx_adminmenublocklink.init(); });