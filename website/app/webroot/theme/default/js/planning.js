var nx_plannif = {
    ...
    init: function(){
        ...
        /* Clic sur une date */
            $('td.date_label').click(function(){
                $(this).parents("tr").find('td:has(:input[type="checkbox"])').each(function(){
                    nx_plannif.clickHoraire($(this), true, true);
                });
            });
        ...
        $('td:has(:input[type="checkbox"])').mouseover(function(){
            if (nx_plannif.ondrag == true){
                nx_plannif.clickHoraire($(this), false, false)
            }
        });
        $('td:has(:input[type="checkbox"])').mousedown(function(){
            nx_plannif.ondrag = true;
        });
        $('td:has(:input[type="checkbox"])').mouseup(function(){
            nx_plannif.ondrag = false;
        });
    ...
    clickHoraire: function(tdElement, with_uncheck, force_check){
        var checkbox = tdElement.find("input[type=checkbox]");
        if (force_check == true){
            checkbox.prop('checked',true);
            tdElement.addClass("date_checked");
        }else{
            var checked = checkbox.is(":checked");
            if(checked && with_uncheck){
...
$(document).ready(function(){ nx_plannif.init(); });