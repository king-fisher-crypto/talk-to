<?php
	...
	if(isset($_GET['invoice_id']))$_SESSION['invoice_id'] = $_GET['invoice_id'];
...
if($_SESSION['invoice_id'] )
	$result_invoice = $mysqli->query("SELECT * FROM  invoice_agents WHERE order_id = '{$_SESSION['invoice_id']}'");
else
	$result_invoice = $mysqli->query("SELECT * FROM  invoice_agents WHERE user_id = '{$_SESSION['fact_agent']}' and date_max = '{$session_date_max}' and status >=0");// and date_min >= '{$session_date_min}'
//var_dump("SELECT * FROM  invoice_agents WHERE user_id = '{$_SESSION['fact_agent']}' and date_max = '{$session_date_max}' and status >=0");exit;
...
$libelle = $row_invoice['society_name'];
...
//check avoir
if($row_invoice['id']){
	$result_voucher = $mysqli->query("SELECT * FROM  invoice_voucher_agents WHERE invoice_id = '{$row_invoice['id']}'");
	$row_voucher = $result_voucher->fetch_array(MYSQLI_ASSOC);
}
...
$pdf->fact_dev( "Invoice ", $row_invoice['order_id'] );
...
$address=  $libelle."\n".
                  $row_invoice['society_address']."\n" .
				  $row_invoice['society_postalcode'].' '.$row_invoice['society_city']."\n".
                  $row_invoice['society_country']."\n";


$address .= $row_invoice['society_num']."\n";

$pdf->addClientAdresse($address);
$pdf->addReglement("Bankwire");
$pdf->addEcheance($date_invoice_txt);
//$pdf->addNumTVA($row['vat_num']);
$pdf->addReference(" ");
$cols=array( "DESCRIPTION"  => 140,
             "Unit price"      => 24,
             "AMOUNT" => 26
              );
$pdf->addCols( $cols);
$cols=array( "DESCRIPTION"  => "L",
             "Unit price"      => "R",
             "AMOUNT" => "R");
...
while($row4 = $result4->fetch_array(MYSQLI_ASSOC)){
	...
	$row4['label'] = str_replace('Frais de service Glassgen pour la periode', 'Fees for Glassgen services (Spiriteo) for the period of', $row4['label']);
	$row4['label'] = str_replace(' au ', ' - ', $row4['label']);
	$line = array( "DESCRIPTION"  => $row4['label'],
				   "Unit price"      => number_format($row4['amount'],2,'.',''),
...
$is_tva = true;
$label_tva = 'TAX';
$remarque = "VAT : Reverse Charge";//"Exonération de TVA - Art 259-1° du CGI";
...
if($row_invoice['vat']){
	$is_tva = true;
	$remarque = "";//"VAT : Ireland ".$row_invoice['vat_tx']."%";
}
$pdf->addRemarque(utf8_decode($remarque));

if($row_voucher){
	$remarque_v ='Credit note issued for this invoice';
	$pdf->addRemarqueVoucher(utf8_decode($remarque_v));
}

$pdf->addTVAs( $row_invoice['amount'], $row_invoice['vat'], $row_invoice['amount_total'], $is_tva,$row_invoice['vat_tx'] );
...
$filename = 'Invoice Glassgen for '.$filename_lastname.' '.$filename_firstname. ' '.$filename_period_month. ' '.$filename_period_an.'.pdf';
	
$pdf->addCadreSolde($sold_info);
$pdf->addCadreEurosFrancs($label_tva, $is_tva,$row_invoice['vat_tx']);
