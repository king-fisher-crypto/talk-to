...
//get order society
	...
	$date_files = date('Y-m-d 00:00:00', strtotime('-1 month'));
	$dx = new DateTime($date_files);
	$date_file = $dx->format('Y-m-d 23:59:59');

//-1 = 2 mois en arriere
//0 = mois dernier

	$date = date('Y-m-d 00:00:00', strtotime('-0 month'));
...
$listing_utcdec = array(
	...
	$dfile = new DateTime($date_file);
		
	$date_filename = clone($dfile);
...
//load data facture
$result_invoice = $mysqli->query("SELECT I.* FROM  invoice_agents I, users U WHERE I.user_id = U.id and I.date_add >= '{$session_date_min}' and I.date_add <= '{$session_date_max}'");
//var_dump("SELECT I.* FROM  invoice_agents I, users U WHERE I.user_id = U.id and I.date_add >= '{$session_date_min}' and I.date_add <= '{$session_date_max}' and I.status =1");exit;
