...
$datesql = date($_GET['date']);
	$dx2 = new DateTime($datesql);
	$sql_date_min = $dx2->format('Y-m-01 00:00:00');
	$dx2 = new DateTime($datesql);
	$dx2->modify('last day of this month');
	$sql_date_max = $dx2->format('Y-m-d 23:59:59');
...
//load data facture
$result_invoice = $mysqli->query("SELECT I.* FROM  invoice_agents I, users U WHERE I.user_id = U.id and I.order_id >= 3059 order by order_id ");
//var_dump("SELECT I.* FROM  invoice_agents I, users U WHERE I.user_id = U.id and I.date_min >= '{$session_date_min}' and I.date_max <= '{$session_date_max}' and I.status =1");
...
$filename_period_month = $date_filename->format('m');
	...
	$pdf->fact_dev( "DEBIT NOTE ", $row_invoice['order_id'] );
