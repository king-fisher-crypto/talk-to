...
//load data facture
$result_invoicel = $mysqli->query("SELECT * FROM  invoice_voucher_agents WHERE invoice_id = -2 and amount > 0 order by id ");
while($row_invoicel = $result_invoicel->fetch_array(MYSQLI_ASSOC)){

	
	$result_fact = $mysqli->query("SELECT * from  invoice_voucher_agents WHERE id = '{$row_invoicel['id']}'");
...
$y    = 109;
    ...
    $line = array( "DESCRIPTION"  => utf8_decode('Avoir sur facture numéro '.$row_invoice2['order_id']),
   ...
   if($row_invoice['invoice_id'] == -1){
    	$line = array( "DESCRIPTION"  => utf8_decode('Over charge from period 06/2019 to 03/2020'),
				   "Unit price"      => number_format($row_invoice['amount'],2,'.',''),
				   "AMOUNT" => number_format($row_invoice['amount'],2,'.',''));
		$size = $pdf->addLine( $y, $line );
		$y   += $size + 2;
		...
		if($row_invoice['invoice_id'] == -2){
			$result_line = $mysqli->query("SELECT * from  invoice_voucher_agent_details WHERE invoice_voucher_id = '{$row_invoice['id']}'");
   			while($row_line = $result_line->fetch_array(MYSQLI_ASSOC)){
				$line = array( "DESCRIPTION"  => utf8_decode('To adjust IN '.$row_line['invoice_order_id'].', being unit price overbilled at '.$row_line['old_amount'].' EUR instead of  '.$row_line['new_amount'].' EUR'),
				   "Unit price"      => number_format($row_line['unit_price'],2,'.',''),
				   "AMOUNT" => number_format($row_line['unit_price'],2,'.',''));
				$size = $pdf->addLine( $y, $line );
				$y   += $size + 2;
				$num ++;
			}
...
$pdf->addCadreEurosFrancs('TAX', $is_tva,$vat_tx );
