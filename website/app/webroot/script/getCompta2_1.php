					...
					foreach($tab_ids as $data){
						if(is_array($data) && $data['id']){
							...
							if($user_credit_id){
								...
								if($roworderold){
									$prepaid_old += $data['seconds'] * $rowcreditprice['price'];	
								}else{
...
var_dump('audiotel : '.number_format($prepaid,2));
var_dump('audiotel stripe : '.number_format($prepaid_stripe,2));
var_dump('audiotel stripe euro : '.number_format($prepaid_stripe_euro,2));
var_dump('audiotel stripe chf : '.number_format($prepaid_stripe_chf,2));
var_dump('audiotel stripe dollar : '.number_format($prepaid_stripe_dollar,2));
var_dump('audiotel paypal : '.number_format($prepaid_paypal,2));
var_dump('audiotel paypal euro : '.number_format($prepaid_paypal_euro,2));
var_dump('audiotel paypal chf : '.number_format($prepaid_paypal_chf,2));
var_dump('audiotel paypal dollar : '.number_format($prepaid_paypal_dollar,2));


var_dump('audiotel unknow : '.number_format($prepaid_unknow,2));
var_dump('audiotel old : '.number_format($prepaid_old,2));
