<?PHP
exit;
//Spiriteo Compress image script
...
$media_dir = '/media/photo';
$dir = __DIR__;
//$files = glob($dir. $media_dir.'/*.jpg');
$files_dir = recursive_read_dir($dir. $media_dir);
$files = array_flatten($files_dir);
$imagecache = new ImageCache();
$imagecache->cached_image_directory = $dir. $media_dir;

foreach($files as $image) {
	if(substr_count($image,'.jpg')){
		//var_dump('IMG->' .$image);
		
		$time = filemtime ( $image );
		if ($time <= (strtotime('-24 hours')-120)) {
		
			//backup 
			$image_backup = str_replace('.jpg','_old.jpg',$image);
			copy($image, $image_backup);	
			$dir_img = dirname($image);

			$cached_src = $imagecache->cache( $image );
			//var_dump('IMG cache->' .$cached_src);
			unlink($image);
			$img = str_replace('https://www.talkappdev.com/app/webroot'.$media_dir,$dir.$media_dir,$cached_src);
			//var_dump('IMG copy->' .$img);
			if($img != $image ){
				if(!rename($img, $image)){
					rename($image_backup, $image);
				}else{
					unlink($image_backup);
				}
			}
