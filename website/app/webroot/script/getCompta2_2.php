...
$resultexpert = $mysqli->query("SELECT * from  users WHERE role ='agent' order by id");
while($rowexpert = $resultexpert->fetch_array(MYSQLI_ASSOC)){

$resultfact = $mysqli->query("SELECT * from  invoice_agents WHERE date_add >= '".$fact_date_debut."' and date_add <= '".$fact_date_fin."' and user_id = '".$rowexpert['id']."' order by id");
$rowfact = $resultfact->fetch_array(MYSQLI_ASSOC);
if(!$rowfact){
	
	$resultcomm = $mysqli->query("SELECT U.type_pay,U.ca as ca_devise,U.ca_currency, P.ca, P.price, U.ca_ids from  user_credit_history U, user_pay P WHERE U.date_start >= '".$date_debut."' and U.date_start <= '".$date_fin."' and U.is_factured = 1 and U.agent_id = '".$rowexpert['id']."' and P.id_user_credit_history = U.user_credit_history order by U.user_credit_history");
