...
while($roworder = $resultorder->fetch_array(MYSQLI_ASSOC)){
  ...
  $second_price = $roworder['total'] / ($roworder['product_credits'] + $roworder['voucher_credits']);
  $old_second_price = $roworder['total'] / ($roworder['product_credits'] );
  if($rowcomm){
   if($rowcomm['credits'] <  ($roworder['voucher_credits'] + $roworder['product_credits'])){
     
     
     $new_ca = $rowcomm['credits'] * $second_price;
     var_dump($rowcomm['ca']. ' -> '.$new_ca);
  ...
  if($ca > 0)
    $table[$date_ex][$rowcomm['agent_id']] += $ca;
      ...
      $secondbuy =  $roworder['voucher_credits'] + $roworder['product_credits'] - $rowcomm['credits'];
       if($secondbuy >= $rowcomm2['credits']){
        $new_ca = $rowcomm2['credits'] * $second_price;
        var_dump($rowcomm2['ca']. ' -> '.$new_ca);
        if($roworder['currency'] == '$')$ca = $ca * $tx_dollar;
        if($roworder['currency'] == 'CHF')$ca = $ca * $tx_chf;
        ...
        if($roword['currency'] == 'CHF')$ca = $ca * $tx_chf;
         if($ca > 0)
        $table[$date_ex][$rowcomm2['agent_id']] += $ca;
     }else{
       $new_ca = ($secondbuy  * $second_price) + ( ($rowcomm2['credits'] - $secondbuy ) * $old_second_price ) ;
        var_dump($rowcomm2['ca']. ' -> '.$new_ca);
        if($roworder['currency'] == '$')$ca = $ca * $tx_dollar;
        if($roworder['currency'] == 'CHF')$ca = $ca * $tx_chf;
        $dx = new DateTime($rowcomm2['date_start']);
     ...
     //comm prend en compte 100% achat
       $new_ca = $rowcomm['credits'] * $second_price;
      var_dump($rowcomm['ca']. ' -> '.$new_ca);
      if($roworder['currency'] == '$')$ca = $ca * $tx_dollar;
    if($roworder['currency'] == 'CHF')$ca = $ca * $tx_chf;
