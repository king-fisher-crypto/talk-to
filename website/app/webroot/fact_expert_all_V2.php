...
//load data facture
$result_invoice = $mysqli->query("SELECT I.* FROM  invoice_agents2 I, users U WHERE I.user_id = U.id and I.date_add >= '{$sql_date_min}' and I.date_add <= '{$sql_date_max}' and I.status >= 0 ");
...
$row4['label'] = str_replace('Frais de service Glassgen pour la periode', 'Fees for Glassgen services (Spiriteo) for the period of', $row4['label']);
	$row4['label'] = str_replace(' au ', ' - ', $row4['label']);
		$line = array( "DESCRIPTION"  => $row4['label'],
					   "Unit price"      => number_format($row_invoice['amount'],2,'.',''),
					   "AMOUNT" => number_format($row_invoice['amount'],2,'.',''));
