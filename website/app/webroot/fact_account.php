...
//error_reporting(E_ALL); 
	...
	if(isset($_GET['a']))$id_com_admin = base64_decode($_GET['a']);
	if(isset($_GET['c']))$id_com = base64_decode($_GET['c']);


	//recup id comm 
	if($id_com_admin > 0){
		$result = $mysqli->query("SELECT * from user_credit_history WHERE user_credit_history = '{$id_com_admin}'");
    	$row = $result->fetch_array(MYSQLI_ASSOC);
		
		$result = $mysqli->query("SELECT * from user_credit_last_histories WHERE sessionid = '{$row['sessionid']}' and media = '{$row['media']}' and agent_id = '{$row['agent_id']}' and date_start = '{$row['date_start']}'");
    	$row = $result->fetch_array(MYSQLI_ASSOC);
		$id_com = $row['user_credit_last_history'];
	}

	if(!isset($id_com))exit;
	
	
	$result = $mysqli->query("SELECT * from invoice_accounts WHERE 	user_credit_last_history = '{$id_com}'");
    $row = $result->fetch_array(MYSQLI_ASSOC);
...
if($row_agent['societe'] && $row_agent['societe_adress']){
	if($row_agent['societe_cp'])$row_agent['societe_ville'] = $row_agent['societe_cp'].' '.($row_agent['societe_ville']);
...
$address .= ($row_agent['address'])."\n" .
                  $row_agent['postalcode'].' '.($row_agent['city'])."\n".
                  $rorow_agentw['phone_number']."\n";
				  //$row_agent['email']."\n";
}
