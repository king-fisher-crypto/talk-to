...
function addCadreTVAs($params)
{
	...
	$this->SetXY( $r1, $y1 + 2);
	if($params['HIPAY']){
		$this->Cell(10,4, 'HIPAY ACCOUNT : '.$params['HIPAY']);
		$this->SetXY( $r1, $y1 + 6);	
	}else{
		if($params['PAYPAL']){
			$this->Cell(10,4, 'PAYPAL ACCOUNT : '.$params['PAYPAL']);
			$this->SetXY( $r1, $y1 + 6);	
		}else{
			...
			$this->SetXY( $r1, $y1 + 14);
			$this->Cell(10,4, 'BANK : '.$params['BANK'].' '.$params['BANK_ADR']);
		}
	}
...
function addCadreEurosFrancs($label_tva = 'TVA', $is_tva = true, $tx_rate = 0)
{
	$r1  = $this->w - 70;
	$r2  = $r1 + 60;
	$y1  = $this->h - 40;
...
//	$this->SetFont( "Arial", "B", 6);
	...
	if($is_tva){
		$this->SetXY( $r1, $y1+10 );
		$this->Cell(20,4, $label_tva.' ('.$tx_rate.'%)', 0, 0, "C");
		$this->SetXY( $r1, $y1+15 );
...
//                  "qte"     => qte,
//                  "tva"     => code_tva );
function addTVAs( $total_ht, $total_vat, $total , $is_tva= true, $tx_vat = 0)
{
	...
	if($is_tva){
		...
		$this->Cell( 17,4, sprintf("%0.2F", $total), '', '', 'R');
	}else{
		$this->SetXY( $rf, $y1+10 );
		$this->Cell( 17,4, sprintf("%0.2F", $total), '', '', 'R');
	}
