...
function addSociete( $top,$nom, $adresse )
{
	$x1 = 10;
	$y1 = 8;
	...
	$length = $this->GetStringWidth( $top );
	$this->Cell( $length, 2, $top);
	$this->SetXY( $x1, $y1 + 6 );
...
function addCadreEurosFrancs($vat_tx)
{
	$r1  = $this->w - 70;
	$r2  = $r1 + 60;
	$y1  = $this->h - 40;
...
//	$this->SetFont( "Arial", "B", 6);
	...
	$this->SetXY( $r1, $y1+10 );
	$this->Cell(20,4, "TVA".' ('.$vat_tx.'%)', 0, 0, "C");
	$this->SetXY( $r1, $y1+15 );
...
//                  "qte"     => qte,
//                  "tva"     => code_tva );
function addTVAs( $total_ht, $total_vat, $total , $currency)
{
	...
	$this->Cell( 17,4, sprintf("%0.2F", $total).' '.$currency, '', '', 'R');
	//$this->SetXY( $rf, $y1+5 );
