...
function addRemarque($remarque)
{
	...
	$remarque = str_replace('$euro$',EURO,$remarque);
	...
	$y2  = $y1+20;
	$this->SetXY( $r1 , $y1 );
	$this->MultiCell( 0, 4 , $remarque, 0,null);
	//$this->Cell($length,4, $remarque);
}
	
function addRemarqueVoucher($remarque)
{
	$this->SetFont( "Arial", "", 10);
	$length = $this->GetStringWidth( $remarque );
	$r1  = 10;
...
function addCadreTVAs($params)
{
	...
	$this->SetXY( $r1, $y1 + 2);
	if($params['HIPAY']){
		$this->Cell(10,4, 'COMPTE HIPAY : '.$params['HIPAY']);
		$this->SetXY( $r1, $y1 + 6);	
	}else{
		if($params['PAYPAL']){
			$this->Cell(10,4, 'COMPTE PAYPAL : '.$params['PAYPAL']);
			$this->SetXY( $r1, $y1 + 6);	
		}else{
			$this->Cell(10,4, 'RIB : '.$params['RIB']);
			$this->SetXY( $r1, $y1 + 6);
			$this->Cell(10,4, 'IBAN : '.$params['IBAN']);
			$this->SetXY( $r1, $y1 + 10);
			$this->Cell(10,4, 'SWIFT : '.$params['SWIFT']);
			$this->SetXY( $r1, $y1 + 14);
			$this->Cell(10,4, 'BANQUE : '.$params['BANK'].' '.$params['BANK_ADR']);
		}
	}
...
// Ajoute une remarque (en bas, a gauche)
function addCadreSolde($sold)
{
	$this->SetFont( "Arial", "", 10);
	$length = $this->GetStringWidth( $sold );
	$r1  = $this->w - 70;
	$r2  = $r1 + $length;
	$y1  = $this->h - 45.5;
	$y2  = $y1+5;
	$this->SetXY( $r1 , $y1 );
	$this->Cell($length,4,  $sold);
}	
	
// trace le cadre des totaux
function addCadreEurosFrancs($label_tva = 'TVA', $is_tva = true, $deposit = 0)
{
	$r1  = $this->w - 70;
	$r2  = $r1 + 60;
	$y1  = $this->h - 45;
...
//	$this->SetFont( "Arial", "B", 6);
	...
	$hauteur = $y1+10;
	if($is_tva){
		$this->SetXY( $r1, $hauteur );
		$this->Cell(20,4, $label_tva, 0, 0, "C");
		$hauteur += 5;
	}
...
//                  "qte"     => qte,
//                  "tva"     => code_tva );
function addTVAs( $total_ht, $total_vat, $total , $is_tva= true, $deposit = 0)
{
	...
	$this->SetFont( "Arial", "", 8);
	$this->SetXY( $rf, $y1+5 );
	$this->Cell( 17,4, sprintf("%0.2F", $total_ht), '', '', 'R');
	$hauteur = $y1+10;
	if($is_tva){
		$this->SetXY( $rf, $hauteur );
		$this->Cell( 17,4, sprintf("%0.2F", $total_vat), '', '', 'R');
		$hauteur += 5;
	}
	
	if($deposit > 0){
		$this->SetXY( $rf, $hauteur );
		$this->Cell( 17,4, sprintf("%0.2F", $deposit), '', '', 'R');
		$hauteur += 5;
	}
	
	$this->SetXY( $rf, $hauteur );
	$this->Cell( 17,4, sprintf("%0.2F", $total-$deposit), '', '', 'R');
