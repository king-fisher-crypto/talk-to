...
class AppController extends Controller {
	...
	public $layout = 'black_blue_side_bar';
	...
	function beforeRender()
	{
		...
		/* DIFFUSE LE STATUT USER (agent / client ) DS TOUTES LES PAGES */ 
		/* SOFTPEOPLE 20-12-2022 */
		if( !empty($user) ) $this->set( "userRole" , $user['role'] );
		...
		if($this->params['controller'] == 'agents' && $this->params['action'] == 'profil' )$in_page = true;
		//if (!empty($user) && $user['role'] === 'agent' && !$in_page) $this->checkVAT();
...
//	   echo"<br>user['role' = ".$user['role'];exit;
