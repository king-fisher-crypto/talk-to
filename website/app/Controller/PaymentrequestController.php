...
*/
class PaymentrequestController extends PaymentController {
	...
	protected $_render_index = '/Payment/Request/index';
	
    protected $_render_admin_index = '/Payment/Stripe/admin_index';
    protected $_render_validation = '/Payment/Request/validation';
	protected $_render_validation_gift = '/Gifts/validation';
    protected $_render_error = '/Payment/Request/error';
	protected $_url_validation = '/Payment/Request/validation?utm_novverride=1';
    protected $_url_error = '/Payment/Request/error?utm_novverride=1';
	protected $_url_form = '/paymentrequest/submit';
    ...
    {
        $this->payment_name = __('Paiement Pay');
    ...
    public function index()
    {
		...
		$return_url =  Router::url('/', true).'paymentrequest/confirm_payment';
		...
		$this->set('request_amount', $cart['total_price'] * 100 );
		$this->set('request_currency', strtolower($cart['product']['Country']['devise_iso']) );
		$this->set('request_desc', $cart['id_cart'] );
		$this->set('request_firstname', $this->cart_datas['user']['firstname'].' '.$this->cart_datas['user']['lastname'] );
		$this->set('request_return', $return_url );
		$this->set('request_country', $country );
	...
	public function confirm_payment()
    {
		...
		$source_id = $this->Session->read('Request_source');
		$cart_id = $this->Session->read('Request_cart');
		//var_dump($source_id);
    ...
    private function saveLogOrder($stripeLogs = 0)
    {
		...
		$type = 'carte';
		if(isset($stripeLogs->payment_method_details->card->wallet->type))
			$type = $stripeLogs->payment_method_details->card->wallet->type;
		
		$data = array();
		$data['cart_id'] = $cart['id_cart'] ;
		$data['order_id'] = $order_id ;
		$data['id'] = $stripeLogs->id;
		$data['payment_method'] = $type.' '.$stripeLogs->payment_method;
	...
	public function save_cart(){
		...
		$this->layout = false;
			...
			if($requestData['source']){
				$cart = $this->cart_datas;
				$this->Session->write('Request_source',$requestData['source']);
				$this->Session->write('Request_cart',$cart['id_cart']);
