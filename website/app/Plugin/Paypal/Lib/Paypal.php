 ...
 */
	protected $paypalClassicApiVersion = '108.0';
 ...
 * @author Rob Mcvey
 **/
    public function setExpressCheckout($order, $overrideNvpFunction=false, &$parsed=false) {
        try {
            // Build the NVPs
            $nvps = $this->buildExpressCheckoutNvp($order, $overrideNvpFunction);
    ...
    public function nx_verif_post_datas_integrity($params=false)
    {
        // HttpSocket
        if (!$this->HttpSocket) {
            $this->HttpSocket = new HttpSocket();
        ...
        // Make a Http request for a new token
        $response = $this->HttpSocket->post($endPoint , $params);
        return isset($response->body)?trim($response->body):false;
    }
 ...
 * @author Rob Mcvey
 **/
    public function buildExpressCheckoutNvp($order, $overrideNvpFunction=false) {
        if (empty($order) || !is_array($order)) {
            throw new PaypalException(__d('paypal' , 'You must pass a valid order array'));
        }
        ...
        $nvps = array(
			...
			'SOLUTIONTYPE' => 'Sole',
			'LOGOIMG' => 'https://fr.spiriteo.com/theme/default/img/logo.png'
        );
        // Custom field?
        ...
        $nvps['PAYMENTREQUEST_0_ITEMAMT'] = number_format(floatval($nvps['PAYMENTREQUEST_0_ITEMAMT']), 2, '.', '');
        $nvps['PAYMENTREQUEST_0_AMT'] = number_format(floatval($nvps['PAYMENTREQUEST_0_AMT']), 2, '.', '');
        $nvps['L_PAYMENTREQUEST_0_AMT0'] = number_format(floatval($nvps['L_PAYMENTREQUEST_0_AMT0']), 2, '.', '');



        if ($overrideNvpFunction !== false){
            $nvps = call_user_func_array(array($overrideNvpFunction['object'], $overrideNvpFunction['function']), array($nvps));
        }
        return $nvps;
    }

	public function original_buildExpressCheckoutNvp($order) {
		if (empty($order) || !is_array($order)) {
			throw new PaypalException(__d('paypal' , 'You must pass a valid order array'));
		}
		if (!isset($order['return']) || !isset($order['cancel'])) {
 ...
 **/
	public function oAuthTokenUrl() {
		return $this->getRestEndpoint() . '/v2/oauth2/token';
	}
 ...
 **/
	public function chargeStoredCardUrl() {
		return $this->getRestEndpoint() . '/v2/payments/payouts';
	}
 ...
 **/
	public function storeCreditCardUrl() {
		return $this->getRestEndpoint() . '/v2/vault/credit-card';
	}
